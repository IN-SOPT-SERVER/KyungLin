import { Request, Response } from "express";
import { userService } from "../service";

//* 유저 생성
const createUser = async (req: Request, res: Response) => {
  const {name, email, age} = req.body // 비구조 할당

  if(!name || !email || !age) return res.status(400).json({ status: 404, message: "유저 생성 실패" });

  const data = await userService.createUser(name, email, age);
  if (!data) {
    return res.status(400).json({ status: 404, message: "유저 생성 실패" });
  }
  return res.status(200).json({ status: 200, message: "유저 생성 성공", data });
}
//* 유저 전체조회
const getAllUser = async (req: Request, res: Response) => {
  const data = await userService.getAllUser();

  return res.status(200).json({ status: 200, message: "유저 전체 조회 성공", data });
}

//* 유저 정보 수정
const updateUser = async (req: Request, res: Response) => {
  const {name} = req.body;
  const {userId} = req.params
  if(!name) return res.status(400).json({ status: 404, message: "유저 수정 실패" });

  const data = await userService.updateUser(+userId, name);
  if (!data) {
    return res.status(400).json({ status: 404, message: "유저 수정 실패" });
  }
  return res.status(200).json({ status: 200, message: "유저 수정 성공", data });
}

//* 유저 삭제
const deleteUser = async (req: Request, res: Response) => {
  const {userId} = req.params

  const data = await userService.deleteUser(+userId);
  if (!data) {
    return res.status(400).json({ status: 404, message: "유저 삭제 실패" });
  }
  return res.status(200).json({ status: 200, message: "유저 삭제 성공", data });
}

const getUserById = async (req: Request, res: Response) => {
  const { userId } = req.params;

  const data = await userService.getUserById(+userId);  // '+userId'userId를 number로 형변환 해준다.

  if (!data) {
    return res.status(404).json({ status: 404, message: "NOT_FOUND" });
  }
  return res.status(200).json({ status: 200, message: "유저 조회 성공", data });
};

const userController = {
  getUserById,
  getAllUser,
  createUser,
  updateUser,
  deleteUser
};

export default userController;
